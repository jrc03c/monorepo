...
async function dfToJSON(df, filename, axis) {
  ...
  try {
    ...
    fs2.writeFileSync(path2.resolve(filename), out2, "utf8");
...
var ARRAY_QUALIFIER = ".filedb.meta.is-array";
...
${invalidParts.map((part) => "\u2192 " + part).join("\n")}`
    );
  }
  if (key.endsWith("/")) {
...
var FileDB = class {
  constructor(relativePath) {
    ...
    if (relativePath[0] === "~") {
      relativePath = import_node_path.default.join(
        import_node_os.default.homedir(),
        relativePath.substring(1, relativePath.length)
      );
    }
    this.dbPath = import_node_path.default.resolve(relativePath);
    import_node_fs.default.mkdirSync(this.dbPath, { recursive: true });
  }
  existsSync(key) {
    return import_node_fs.default.existsSync(import_node_path.default.join(this.dbPath, "/", standardizeKey(key)));
  }
  exists(key, callback) {
    return new Promise((resolve, reject) => {
      try {
        const out2 = this.existsSync(key);
        if (callback) callback(out2);
        return resolve(out2);
      } catch (e) {
        return reject(e);
      }
  ...
  writeSync(key, value, ignored) {
    ...
    const filePath = import_node_path.default.join(this.dbPath, "/", key);
    ...
    try {
      import_node_fs.default.rmSync(filePath, { recursive: true, force: true });
    } catch (e) {
    }
    if (typeof value === "undefined" || value === null) {
      ...
      import_node_fs.default.writeFileSync(filePath, value, "utf8");
      return true;
    }
  }
  write(key, value, ignored, callback) {
    return new Promise((resolve, reject) => {
      try {
        const out2 = this.writeSync(key, value, ignored);
        if (callback) callback(out2);
        return resolve(out2);
      } catch (e) {
        return reject(e);
  ...
  readSync(key, maxDepth, ignored) {
    ...
    const innerRead = (innerKey, currentDepth) => {
      const filePath = import_node_path.default.join(this.dbPath, "/", innerKey);
      ...
      const stats2 = import_node_fs.default.lstatSync(filePath);
      if (stats2.isFile()) {
        const data = import_node_fs.default.readFileSync(filePath, "utf8");
        let out2 = data;
        try {
          out2 = JSON.parse(data);
        } catch (e) {
        }
        return out2;
      } else {
        ...
        if (currentDepth >= maxDepth) {
          ...
          files.forEach((file) => {
            ...
            if (Object.keys(out2).length >= files.length - failures) {
              isDone = true;
            }
          });
          const { isArray: isArray2, parsedKeys } = (() => {
            const rawKeys = Object.keys(out2);
            if (rawKeys.indexOf(ARRAY_QUALIFIER) > -1) {
              return {
                isArray: true,
                parsedKeys: sort(
                  rawKeys.filter((key2) => key2 !== ARRAY_QUALIFIER).map((key2) => {
                    if (isWholeNumber2(key2)) {
            ...
            for (const key2 of rawKeys) {
              const parsedKey = parseInt(key2);
              if (isWholeNumber2(key2) && parsedKey === previousKey + 1) {
                parsedKeys2.push(parsedKey);
            ...
            return { isArray: true, parsedKeys: parsedKeys2 };
          })();
          if (isArray2) {
            const temp = [];
            parsedKeys.forEach((key2) => temp[key2] = out2[key2]);
            return temp;
          } else {
            return out2;
          }
  ...
  read(key, maxDepth, ignored, callback) {
    return new Promise((resolve, reject) => {
      try {
        const out2 = this.readSync(key, maxDepth, ignored);
        if (callback) callback(out2);
        return resolve(out2);
      } catch (e) {
        return reject(e);
