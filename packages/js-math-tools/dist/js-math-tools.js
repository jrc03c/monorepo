(() => {
  ...
  // src/dataframe/df-get-subset-by-names.mjs
  ...
  // src/series/series-get-subset-by-indices.mjs
  ...
  // src/series/series-get-subset-by-names.mjs
      ...
      if (isObject(x2)) {
        ...
        for (let i = 0; i < keys.length; i++) {
          ...
          if (results2 && results2.length > 0) {
            forEach(
              results2.slice(alreadyStoredThisValue ? 1 : 0),
              (r) => out.push(r)
            );
          }
        }
    ...
    if (xShape[0] === 0) {
      ...
      for (let divider = 1; divider < xShape[0] - 1; divider++) {
        try {
          ...
          const out = map(topLeft, (row, i) => row.concat(topRight[i])).concat(map(bottomLeft, (row, i) => row.concat(bottomRight[i])));
          return out;
        } catch (e) {
        }
      }
  ...
  if (typeof globalThis !== "undefined") {
    globalThis.JSMathTools = index_exports;
  }
})();
