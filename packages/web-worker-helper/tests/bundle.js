(() => {
  ...
  !(async () => {
    ...
    await (async () => {
      ...
      try {
        const helper4 = new WebWorkerHelper(
          "worker-bundle.js",
          { type: "module" }
        );
        const x = Math.random();
        const ytrue = x * 2;
        const ypred = await helper4.exec("double", x);
        if (ypred === ytrue) {
          resultElement.classList.add("success");
        } else {
          resultElement.classList.add("danger");
          resultElement.querySelector("#message").innerHTML += ` (We expected to receive ${ytrue} but instead received ${ypred}!)`;
        }
      } catch (e) {
        resultElement.classList.add("danger");
        resultElement.querySelector("#message").innerHTML = `${description} (ERROR: ${e})`;
      }
    ...
    await (async () => {
      ...
      try {
        ...
        const x = Math.random();
        const ytrue = x * 3;
        const ypred = await helper4.exec("triple-after-a-while", x);
        if (ypred === ytrue) {
          resultElement.classList.add("success");
        } else {
          resultElement.classList.add("danger");
    ...
    await (async () => {
      ...
      try {
        ...
        let progress = 0;
        await helper4.exec("run-progress-callbacks", null, (p) => {
          progress = p;
          resultElement.querySelector("#message").innerHTML = `${description} (${(p * 100).toFixed(2)}%)`;
        });
        if (progress === 1) {
          resultElement.classList.add("success");
          ...
          resultElement.classList.add("danger");
          resultElement.querySelector("#message").innerHTML += ` (We expected to receive a final progress value of 1 but instead received ${progress}!)`;
        }
      } catch (e) {
        resultElement.classList.add("danger");
        resultElement.querySelector("#message").innerHTML = `${description} (ERROR: ${e})`;
    ...
    await (async () => {
      ...
      try {
        ...
        await helper4.exec("does-not-exist");
        resultElement.classList.add("danger");
      } catch (e) {
        resultElement.classList.add("success");
      }
    ...
    await (async () => {
      ...
      try {
        ...
        const promises = [];
        promises.push(helper4.exec("do-thing-1"));
        promises.push(helper4.exec("do-thing-2"));
        promises.push(helper4.exec("do-thing-3"));
    ...
    await (async () => {
      ...
      try {
        ...
        resultElement.classList.add("danger");
        resultElement.querySelector("#message").innerHTML = `${description} (The worker should've been terminated mid-execution, but it wasn't!)`;
      } catch (e) {
        resultElement.classList.add("success");
      }
      resultElement.classList.remove("animated");
