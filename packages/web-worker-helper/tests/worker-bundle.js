(() => {
  ...
  // tests/worker.mjs
  var helper4 = new WebWorkerHelper();
  helper4.on("double", (x) => {
    return x * 2;
  });
  helper4.on("triple-after-a-while", async (x) => {
    await pauseAsync(3e3);
    return x * 3;
  });
  helper4.on("run-progress-callbacks", async (x, progress) => {
    let p = 0;
    while (p < 1) {
      progress(p);
      p += Math.random() / 10;
  ...
  helper4.on("do-thing-1", async () => {
    await pauseAsync(Math.random() * 500 + 500);
    return "Thing 1 is finished!";
  });
  helper4.on("do-thing-2", async () => {
    await pauseAsync(Math.random() * 500 + 500);
    return "Thing 2 is finished!";
  });
  helper4.on("do-thing-3", async () => {
    await pauseAsync(Math.random() * 500 + 500);
    return "Thing 3 is finished!";
  });
  helper4.on("return-after-30-seconds", async () => {
    await pauseAsync(3e4);
  });
})();
