...
async function dfToJSON(df, filename, axis) {
  ...
  try {
    ...
    fs5.writeFileSync(path3.resolve(filename), out2, "utf8");
...
function getFilesDeepSync(dir, depth) {
  ...
  children.forEach((child) => {
    ...
    const stat = (() => {
      try {
        return import_node_fs.default.lstatSync(childPath);
      } catch (e) {
        return null;
      }
    })();
    ...
    if (stat.isFile() || stat.isSymbolicLink()) {
      ...
      getFilesDeepSync(childPath, depth - 1).forEach((d) => out2.push(d));
    }
  });
  return sort(set(out2));
}

// src/apply-inclusions-and-exclusions.mjs
...
function watch(config) {
  ...
  const target = import_node_path2.default.resolve(config.target);
  ...
  if (config.include) {
    if (config.include instanceof RegExp || typeof config.include === "string") {
  ...
  if (config.exclude) {
    if (config.exclude instanceof RegExp || typeof config.exclude === "string") {
  ...
  let files = import_node_fs2.default.statSync(target).isFile() ? [target] : getFilesDeepSync(target);
  ...
  const interval = setInterval(async () => {
    ...
    try {
      newModificationTime = import_node_fs2.default.statSync(file).mtimeMs;
      if (!dict[file]) {
    ...
    if (index >= files.length) {
      first = false;
      index = 0;
      let newFiles = import_node_fs2.default.statSync(target).isFile() ? [target] : getFilesDeepSync(target);
