...
function writeToStdout(message, type, payload, filename) {
  ...
  console.log("-----");
  console.log(
    `${fx.bright(color(type.toUpperCase()))} ${color("[" + (/* @__PURE__ */ new Date()).toUTCString() + "]")}: ${message}` + (payload ? ` See "${filename}" to inspect this message's payload.` : "")
  );
}
var Logger = class {
  logs = [];
  ...
  constructor(options) {
    ...
    this.shouldWriteToDisk = typeof options.shouldWriteToDisk === "undefined" ? this.shouldWriteToDisk : options.shouldWriteToDisk;
    this.shouldWriteToStdout = typeof options.shouldWriteToStdout === "undefined" ? this.shouldWriteToStdout : options.shouldWriteToStdout;
  ...
  getEntryFSPath(entry) {
    if (this.shouldWriteToDisk) {
      if (import_node_fs.default.statSync(this.path).isFile()) {
        return this.path;
      } else {
        ...
        return import_node_path.default.join(this.path, filename);
      }
    } else {
      return "[n/a]";
    }
  ...
  load() {
    if (this.shouldWriteToDisk) {
      ...
      const stat = import_node_fs.default.statSync(this.path);
      if (stat.isFile()) {
        try {
          const raw = import_node_fs.default.readFileSync(this.path, "utf8");
          this.logs = JSON.parse(raw);
        } catch (e) {
          this.logs = [];
        }
      } else {
        const files = import_node_fs.default.readdirSync(this.path);
        try {
          this.logs = files.map(
            (f) => JSON.parse(import_node_fs.default.readFileSync(import_node_path.default.join(this.path, f), "utf8"))
          );
        } catch (e) {
          this.logs = [];
        }
  ...
  log(message, type, payload) {
    const date = (/* @__PURE__ */ new Date()).toJSON();
    const id = makeKey4(8);
  ...
  save() {
    ...
    if (this.shouldWriteToDisk) {
      ...
      const stat = import_node_fs.default.statSync(this.path);
      if (stat.isFile()) {
        import_node_fs.default.writeFileSync(this.path, JSON.stringify(this.logs, null, 2), "utf8");
      } else {
        this.logs.forEach((entry) => {
          const file = this.getEntryFSPath(entry);
          import_node_fs.default.writeFileSync(file, JSON.stringify(entry, null, 2), "utf8");
        });
      }
    }
    this.emit("save");
