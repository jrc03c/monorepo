...
test("tests that the `KMeansNaive` model works correctly", () => {
  !(() => {
  ...
  !(() => {
    ...
    const model1 = new KMeansNaive({ k: 3 })
    model1.fit(xBigInts)

    seed(12345)

    const model2 = new KMeansNaive({ k: 3 })
    model2.fit(xFloats)
  ...
  !(() => {
    ...
    const modelForNaNs = new KMeansNaive({ k: 3 })
