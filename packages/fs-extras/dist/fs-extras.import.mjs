// src/create-file-stream-reader.mjs
...
function createFileStreamReader(file, progress) {
  ...
  const fileSize = progress ? fs.statSync(file).size : void 0;
...
// src/create-file-stream-writer.mjs
...
function getDirsDeepSync(dir, depth) {
  ...
  children.forEach((child) => {
    ...
    const stat = (() => {
      try {
        return fs3.lstatSync(childPath);
      } catch (e) {
        return null;
      }
    })();
    ...
    if (stat.isDirectory()) {
      out2.push(childPath);
      getDirsDeepSync(childPath, depth - 1).forEach((d) => out2.push(d));
    }
  });
  return sort(set(out2));
}

// src/get-files-deep-sync.mjs
...
function getFilesDeepSync(dir, depth) {
  ...
  children.forEach((child) => {
    ...
    const stat = (() => {
      try {
        return fs4.lstatSync(childPath);
      } catch (e) {
        return null;
      }
    })();
...
function findSync(dir, matcher, depth) {
  ...
  const out2 = all.filter((f) => matcher(f));
  return sort(set(out2));
}
...
function findAsync(dir, pattern, depth, callback) {
  return new Promise((resolve, reject) => {
    try {
      const out2 = findSync(dir, pattern, depth);
