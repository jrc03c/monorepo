...
function pathJoin() {
  ...
  const parts = (() => {
    ...
    temp = temp.replace(/^\.\//, "");
    temp = temp.replace(/\/\.$/, "");
    temp = temp.split("/").map((v) => v.trim()).filter((v) => v.length > 0);
    return temp;
  })();
  let out3 = [];
  ...
  out3 = out3.join("/");
  return (startsWithForwardSlash && !out3.startsWith("..") ? "/" : "") + out3;
}

// src/helpers/absolutify-url.mjs
var extensions = [
  "a",
  "aac",
  "accdb",
...
function absolutifyUrl(currentUrl, targetUrl) {
  ...
  if (targetUrl.startsWith("/")) {
    const temp2 = new URL(currentUrl);
    return temp2.protocol + "//" + pathJoin(temp2.hostname + (temp2.port ? ":" + temp2.port : ""), targetUrl);
  }
  const temp = new URL(currentUrl);
  const updatedPath = extensions.some((ext) => temp.pathname.includes("." + ext)) ? temp.pathname.split("/").slice(0, -1).join("/") : temp.pathname;
  return temp.protocol + "//" + pathJoin(
...
// src/helpers/extract-urls-from-css.mjs
  ...
  return sort(
    set(
      (css.match(/url\(.*?\)/g) || []).map(
        (v) => absolutifyUrl(
          url,
          v.replace("url(", "").replace(")", "").replaceAll("'", "").replaceAll('"', "")
        )
      )
    )
  );
...
// src/helpers/fetch-with-timeout.mjs
...
// src/helpers/get-all-elements.mjs
function getAllElements(dom, root) {
  ...
  if (root.children) {
    Array.from(root.children).forEach((child) => {
      out3.push(...getAllElements(dom, child));
    });
  }
  return flatten(out3);
}

// src/helpers/get-attribute-urls.mjs
function getAttributeUrls(url, el, included) {
  ...
  return sort(
    set(
      el.getAttributeNames().filter((a) => !included || included.includes(a)).map((a) => el.getAttribute(a)).map(
        (v) => v.toString().split(";").map(
          (v2) => v2.split(",").map(
            (v3) => v3.split(" ").map(
              (v4) => v4.split(/\s/g).map((v5) => absolutifyUrl(url, v5.trim()))
            )
          )
        )
      )
...
// src/helpers/robots-config/index.mjs
var RobotsConfig = class _RobotsConfig {
  botRules = {};
  raw = "";
  sitemapUrls = [];
  static parse(raw) {
    const regexifyRuleString = (rule) => {
      return rule === "*" ? rule : new RegExp(rule.replaceAll(".", "\\.").replaceAll("*", "(.*?)"));
    };
    const lines = raw.split("\n").map((line) => line.trim()).filter((line) => line.length > 0 && !line.match(/^\s*?#/g));
    const config = new _RobotsConfig();
    config.raw = raw;
    let agent = null;
    for (const line of lines) {
      ...
      if (agent) {
        if (lowerLine.match(/^\s*allow\s*:/gim) || lowerLine.match(/^\s*disallow\s*:/gim)) {
          const key = lowerLine.split(":")[0].trim();
          const rule = line.split(":").slice(1).join(":").trim();
          const patterns = rule.split(",").map((rule2) => regexifyRuleString(rule2));
          if (!config.botRules[agent][key]) {
            config.botRules[agent][key] = [];
          }
          config.botRules[agent][key] = config.botRules[agent][key].concat(patterns);
        }
      }
  ...
  toString() {
    ...
    Object.keys(this.botRules).forEach((agent) => {
      ...
      Object.keys(this.botRules[agent]).forEach((key) => {
        botRulesOut[agent][key] = this.botRules[agent][key].map(
          (v) => stringify(v)
        );
      });
    });
...
// src/index.mjs
var virtualConsole = new import_jsdom.VirtualConsole();
virtualConsole.on("error", () => {
});
var WebCrawler = class _WebCrawler {
  ...
  constructor(options) {
    ...
    this.shouldHonorBotRules = typeof options.shouldHonorBotRules === "undefined" ? this.shouldHonorBotRules : options.shouldHonorBotRules;
    this.shouldOnlyFollowSitemap = typeof options.shouldOnlyFollowSitemap === "undefined" ? this.shouldOnlyFollowSitemap : options.shouldOnlyFollowSitemap;
  }
      ...
      config = await (async () => {
        ...
        try {
          ...
          if (response.status === 200) {
            ...
            const config2 = RobotsConfig.parse(raw);
            return config2;
          } else {
          ...
          try {
            ...
            if (response.status === 200) {
              ...
              if (sitemapUrl.toLowerCase().endsWith(".xml")) {
                try {
                  const dom = new import_jsdom.JSDOM(raw, {
              ...
              if (sitemapUrl.toLowerCase().endsWith(".txt")) {
                raw.split("\n").map((v) => v.trim()).filter((v) => v.length > 0).forEach((v) => toCrawl2.push(v));
              }
            } else {
  ...
  async start(url) {
    ...
    while (this.frontier.length > 0) {
      while (this.isPaused) {
        await pauseAsync(this.delay);
      }
      const url2 = this.frontier.shift();
      if (this.visited.includes(url2)) {
        this.emit("skip-url", { url: url2 });
        continue;
      } else {
        this.visited.push(url2);
      }
      if (!await this.filter(url2)) {
        this.emit("filter-url", { url: url2 });
        continue;
      }
      const domain = new URL(url2).hostname;
      let config = await this.getDomainConfiguration(domain);
      if (this.shouldHonorBotRules && !config.isAllowed("*", new URL(url2).pathname)) {
        this.emit("disallow-url", {
          reason: _WebCrawler.DisallowReasons.DISALLOWED_BY_BOT_RULES,
          url: url2
        });
        continue;
      }
      try {
        this.emit("before-crawl", { url: url2 });
        const response = await fetchWithTimeout(url2, null, this.requestTimeout);
        const raw = await response.text();
        this.emit("fetch", { raw, response, url: url2 });
        if (response.status === 200) {
          if (url2.toLowerCase().endsWith(".css")) {
            extractUrlsFromCss(url2, raw).forEach(
              (newUrl) => this.addUrlToFrontier(newUrl)
            );
          } else {
            if (this.shouldHonorBotRules) {
              ...
              if (xRobotsTag && xRobotsTag === "noindex") {
                this.emit("disallow-url", {
                  reason: _WebCrawler.DisallowReasons.DISALLOWED_BY_RESPONSE_HEADER,
                  url: url2
                });
                await pauseAsync(this.delay);
                continue;
            ...
            try {
              const dom = new import_jsdom.JSDOM(raw, { virtualConsole });
              if (this.shouldHonorBotRules) {
                const metas = Array.from(
                  dom.window.document.querySelectorAll("meta")
                );
                const meta = metas.find((meta2) => meta2.name === "robots");
                if (meta && meta.content.includes("noindex")) {
                  this.emit("disallow-url", {
                    reason: _WebCrawler.DisallowReasons.DISALLOWED_BY_META_TAG,
                    url: url2
                  });
                  await pauseAsync(this.delay);
                  continue;
              ...
              if (!this.shouldOnlyFollowSitemap || config.sitemapUrls.length === 0) {
                const validAttributes = ["href", "src"];
                getAllUrls(url2, dom, validAttributes).forEach(
                  (newUrl) => this.addUrlToFrontier(newUrl)
                );
              }
              this.emit("after-crawl", { dom, raw, response, url: url2 });
              await pauseAsync(this.delay);
            } catch (e) {
              const message = `Error processing contents of URL: (${url2}) ${e}`;
              this.emit("error", { message, url: url2 });
            }
          }
        } else {
          const message = `Error fetching URL: (${response.status}) ${url2}`;
          this.emit("error", { message, response, url: url2 });
        }
      } catch (e) {
        const message = `Error fetching URL: (${url2}) ${e}`;
        this.emit("error", { message, url: url2 });
      }
    }
